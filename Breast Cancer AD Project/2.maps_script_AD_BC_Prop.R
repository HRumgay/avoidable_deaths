...
AD_props%>%select(country_label)%>%distinct()
...
ggplot() + 
  ...
  theme(legend.key.width= unit(2.6, "cm"), 
        ...
        legend.background = element_rect(fill="transparent"),
        plot.margin = unit(c(0,0,0,0),"lines"))+
  scale_fill_manual(name = "Proportion of treatable deaths (%), median reference",
                    values= colors_green_GCO,
                    labels= labels_leg, 
                    na.value = "#cccccc",
                    drop=FALSE)+
  guides(fill = guide_legend(reverse = FALSE))+
  scale_color_manual(values=c("grey100", "grey10"))+
  scale_linetype_manual(values=c("solid", "11"))->AD_map_median

ggsave("\\\\Inti\\cin\\Studies\\Survival\\SurvCan\\Data\\Oliver_Langselius\\Breast Cancer\\Figures\\map_AD_breast_median.pdf",width = 40, height = 30, pointsize = 12) 
...
ggplot() + 
  ...
  theme(legend.key.width= unit(2.6, "cm"), 
        ...
        legend.background = element_rect(fill="transparent"),
        plot.margin = unit(c(0,0,0,0),"lines"))+
  scale_fill_manual(name = "Proportion of treatable deaths (%), maximum reference",
                    values= colors_green_GCO,
                    labels= labels_leg, 
                    na.value = "#cccccc",
                    drop=FALSE)+
...
ggarrange( AD_map_93, AD_map_median, AD_map_max,
          labels = c("a)", "b)", "c)"),
          ncol = 1, nrow = 3,
          font.label = list(size = 60, color = "black"))


ggsave("\\\\Inti\\cin\\Studies\\Survival\\SurvCan\\Data\\Oliver_Langselius\\Breast Cancer\\Figures\\map_AD_prop.pdf", width = 40, height =67, limitsize = FALSE) 
