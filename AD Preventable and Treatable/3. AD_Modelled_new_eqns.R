...
Avoidable_Deaths_Simulated_All3<-Simulated_Data_PAF_All%>%
  ...
  dplyr::mutate(AD_prev= total_overall * af.comb *ES* (1 - rel_surv))%>%#From scenario 2
  dplyr::mutate(AD_treat=total_overall * (surv_ref-rel_surv) * ES)%>% #Scenario 2
  dplyr::mutate(AD_treat_not_prev = (1-af.comb) * total_overall * (surv_ref-rel_surv) * ES)%>% #scenario 1
  dplyr::mutate(AD_unavoid =(1-(rel_surv*ES)-af.comb*ES*(1-rel_surv))* total_overall)%>% #This one removed the PAF term
  dplyr::mutate(Expect_deaths=AD_prev+AD_treat_not_prev+AD_unavoid)%>%
...
Avoidable_Deaths_Simulated_All_age_cat_overall<-Avoidable_Deaths_Simulated_All_age_cat%>% #Object to plot overall age standardized on world maps 
  ...
  select( "country_code","country_label","cancer_code", "cancer", 
          ...
          "total_deaths"  )%>%
  mutate(across(5:5,round,0 ))%>%
  mutate(across(6:6, round,4)*100)%>% 
  mutate(across(7:7,round,0 ))%>%
  mutate(across(8:8, round,4)*100)%>% 
  mutate(across(9:9,round,0 ))%>%
  mutate(across(10:10, round,4)*100)%>% 
  mutate(across(11:11,round,0 ))%>%
  mutate(across(12:12, round,4)*100)%>% 
  mutate(across(13:13,round,0 ))%>%
  mutate(across(14:14, round,4)*100)%>% 
  mutate(across(15:15,round,0 ))
#By cancer site
