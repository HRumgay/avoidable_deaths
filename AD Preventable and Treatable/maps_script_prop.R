...
# load paf results
premature <- as.data.table(dat)

# load id for each country
dict_id <-  as.data.table(read.csv("//inti/cin/Xchange/Mathieu/data/_shape/id_OMS_official_general_map.csv", sep=","))
dict_id %>% dplyr::select(-country_label)-> dict_id

# merge paf data with dict_id
premature <- merge(premature, dict_id, by = c("country_code"), all.x = T) 
...
shp_temp <- readOGR(dsn="//inti/cin/Xchange/Mathieu/data/_shape/OMS_official_general", layer="general_2013")
...
shp_temp <- readOGR(dsn="//inti/cin/Xchange/Mathieu/data/_shape/OMS_official_general", layer="maskpoly_general_2013")
shp_temp <- spTransform(shp_temp, CRS("+proj=eck3"))
df_poly <- fortify(shp_temp)
int_map_index <- c(1:nrow(df_poly))
df_poly$int_map_index <- int_map_index

df_poly_layout <-  read.csv("//inti/cin/Xchange/Mathieu/data/_shape/id_OMS_official_general_poly.csv", sep=",")
...
shp_temp <- readOGR(dsn="//inti/cin/Xchange/Mathieu/data/_shape/OMS_official_general", layer="maskline_general_2013")
shp_temp <- spTransform(shp_temp, CRS("+proj=eck3"))
df_line <- fortify(shp_temp)
int_map_index <- c(1:nrow(df_line))
df_line$int_map_index <- int_map_index

df_line_layout <-  read.csv("//inti/cin/Xchange/Mathieu/data/_shape/id_OMS_official_general_line.csv", sep=",")
...
#--- maps for premature mortality in males ----

allc <- premature[sex == 1,][type==1,]

break_quantile <- quantile(allc$avoid_death_percent, probs = seq(0, 1, by = 0.125), na.rm = T)

allc$cutpoint <- cut(allc$avoid_death_percent, breaks = rev(break_quantile), include.lowest = TRUE)
table(allc$cutpoint)
...
ggplot() + 
  ...
  scale_fill_manual(name = "% of total",
...
ggsave("map_male_all_cancers_prop.pdf",width = 40, height = 30, pointsize = 12)    

#--- maps for premature mortality in females ----

allc <- premature[sex == 2,][type==1,]
...
ggsave("map_female_all_cancers_prop.pdf",width = 40, height = 30, pointsize = 12) 

#--- maps for premature mortality in both sexes ----

allc <- premature[sex == 0,][type==1,]
...
ggplot() + 
  ...
  geom_path(data = df_line[df_line$line_color != 0, ],
            ...
            size = 0.4,
            show.legend=FALSE)+
       labs(title = paste0("World map of proportion of premature mortality in 2020, both sexes"))+
  coord_equal() + 
  theme_opts +
  theme(legend.key.width= unit(2.6, "cm"), #1.5 before
        legend.key.height= unit(1.4, "cm"),
...
ggsave("map_both_sexes_all_cancers_prop.pdf",width = 40, height = 30, pointsize = 12) 
