...
rdata$surv_yydd <- rdata$time/365.241
...
## Dataset
MyData <- Thai_Surv ## You can change the dataset here, e.g.: subset(rdata, sex01==0)
...
## "expected" argument)
MyData2 <- calcExpect(time="surv_yydd",event="event1",ratetable=ratetablepop, rmap=list(age=age*365.241, year=year, sex=sex), data=MyData)

## Model for the overall survival
ModTot <- mexhaz(Surv(time=surv_yydd,event=cens)~1, data=MyData2,base="exp.ns",knots=quantile(MyData2[MyData2$cens==1,]$surv_yydd,probs=c(1:2/3)))

## Model for the excess hazard
ModExc <- mexhaz(Surv(time=surv_yydd,event=cens)~1, data=MyData2,expected="mua",base="exp.ns",knots=quantile(MyData2[MyData2$cens==1,]$surv_yydd,probs=c(1:2/3)))
...
for (i in 1:1000){
  ...
  Temp <- calcExpect(time="timeFix",event="cens",ratetable=ratetablepop,rmap=list(age=age*365.241,year=year_dg,sex=sex),data=MyData)
